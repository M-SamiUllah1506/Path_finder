...
function GraphView({ graph, width = 800, height = 500, onAddNode, onSelectNode, selectedNodes = [], path = [], highlighted = new Set() }) {
  ...
  return (
    <svg ref={svgRef} width={width} height={height} onClick={handleSvgClick} onMouseMove={onMouseMove} onMouseUp={onMouseUp} style={{ border: '1px solid #ccc', background: '#fff' }}>
      {edges.map((e, i) => {
        ...
        return <line key={i} x1={a.x} y1={a.y} x2={b.x} y2={b.y} stroke={on ? '#ff6b6b' : '#999'} strokeWidth={on ? 4 : 2} />;
      })}
      ...
      {nodes.map((n) => (
        <g key={n.id} transform={`translate(${n.x},${n.y})`} onMouseDown={(e) => startDrag(n.id, e)} onClick={(e) => { e.stopPropagation(); onSelectNode && onSelectNode(n.id); }} style={{ cursor: 'pointer' }}>
          <circle r={isOnPath(n.id) ? 10 : 7} fill={selectedNodes.includes(n.id) ? '#ffd54f' : isOnPath(n.id) ? '#ff6b6b' : '#3f51b5'} stroke="#222" strokeWidth={1} />
